<template>
  <div class="receivedOrdersList-container" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy"
       infinite-scroll-distance="1">
    <order-component :key="index" v-for="(item,index) in orderList" :order="item"
                     class="order-content" @confirmorder="confirmOrderHandler"></order-component>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      confirmOrderHandler(e) {
        this.orderList = this.orderList.filter(item => item.orderId !== e)
      }
    },
    created() {
      this._init()
